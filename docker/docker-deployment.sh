...
function export_version() {
    export UBKG_DOWNLOAD_VERSION=$(tr -d "\n\r" < ../VERSION | xargs)
    echo "UBKG_DOWNLOAD_VERSION: $UBKG_DOWNLOAD_VERSION"
}
...
if [[ "$1" != "start" && "$1" != "stop" && "$1" != "down" ]]; then
    ...
    if [ "$1" = "start" ]; then
        docker-compose -f docker-compose.yml -f docker-compose.deployment.yml -p ubkg-download up -d
    elif [ "$1" = "stop" ]; then
        docker-compose -f docker-compose.yml -f docker-compose.deployment.yml -p ubkg-download stop
    elif [ "$1" = "down" ]; then
        docker-compose -f docker-compose.yml -f docker-compose.deployment.yml -p ubkg-download down
    fi
fi